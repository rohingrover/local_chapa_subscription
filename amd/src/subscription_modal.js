...
define(['jquery', 'core/modal_factory', 'core/modal_events', 'core/ajax'], function($, ModalFactory, ModalEvents, Ajax) {
    ...
    var SubscriptionModal = {
        ...
        showModal: function(options) {
            ...
            ModalFactory.create({
                ...
                modal.getRoot().on(ModalEvents.save, function() {
                    ...
                    var target = (SubscriptionModal._config && SubscriptionModal._config.subscribeUrl) || (M.cfg.wwwroot + '/local/chapa_subscription/modern_subscribe.php');
        ...
        createModalContent: function(config) {
            ...
            content += '<p class="price">' + (config.planPrices?.basic || 249) + ' ETB/month</p>';
            ...
            content += '<p class="price">' + (config.planPrices?.standard || 299) + ' ETB/month</p>';
            content += '<ul class="plan-features">';
            ...
            content += '<p class="price">' + (config.planPrices?.premium || 349) + ' ETB/month</p>';
            content += '<ul class="plan-features">';
        ...
        init: function(config) {
            var self = this;
            SubscriptionModal._config = config || {};
            var currentPlan = (config && config.currentPlan) || '';
            var planHierarchy = (config && config.planHierarchy) || { basic: 1, standard: 2, premium: 3 };
            ...
            // Detect Moodle native cohort restriction text and make it clickable
            // e.g., "Not available unless: You belong to Basic Plan Cohort"
            $(document).ready(function() {
                $(".availabilityinfo, .availabilityinfo:not(.processed)").each(function() {
                    var $container = $(this);
                    var text = $container.text();
                    var match = text.match(/You belong to\s+(Basic|Standard|Premium) Plan Cohort/i);
                    if (match) {
                        var requiredPlan = match[1].toLowerCase();
                        var isUpgrade = false;
                        if (currentPlan) {
                            isUpgrade = (planHierarchy[currentPlan] || 0) < (planHierarchy[requiredPlan] || 0);
                        }
                        var $btn = $('<a href="#" class="cohort-restriction-message processed"></a>');
                        $btn.attr('data-required-plan', requiredPlan.charAt(0).toUpperCase() + requiredPlan.slice(1));
                        $btn.attr('data-is-upgrade', isUpgrade);
                        $btn.attr('data-current-plan', currentPlan || 'Free Preview');
                        $btn.append($container.html());
